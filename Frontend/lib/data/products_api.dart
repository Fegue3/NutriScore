...
class ProductSummary {
  ...
  factory ProductSummary.fromJson(Map<String, dynamic> j) => ProductSummary(
    ...
    nutriScore: _upper(
      j['nutriScore'] ?? j['nutriscore'] ?? j['nutriscore_grade'],
    ),
    energyKcal100g: _i(
      j['energyKcal100g'] ??
          j['energyKcal_100g'] ??
...
class ProductDetail {
  ...
  factory ProductDetail.fromJson(Map<String, dynamic> j) => ProductDetail(
    ...
    // porção
    kcalServ: _i(
      j['energyKcal_serv'] ?? j['energy-kcal_serving'] ?? j['kcal_serving'],
    ),
    ...
    isFavorite: (j['isFavorited'] ?? j['isFavorite'] ?? j['favorited']) == true,
  );
}
...
class ProductsApi {
  ...
  Future<List<ProductSummary>> suggest(String q, {int limit = 8}) async {
    try {
      ...
      final items = (map['items'] as List? ?? const [])
          .map(
            (e) => ProductSummary.fromJson((e as Map).cast<String, dynamic>()),
          )
          .toList();
      return items;
    } on DioException catch (e) {
  ...
  searchHybrid(String q, {int page = 1, int pageSize = 20}) async {
    try {
      final uri = Uri.parse('$baseUrl$pathPrefix').replace(
  ...
  searchConfirm(String q, {int page = 1, int pageSize = 20}) async {
    try {
      final uri = Uri.parse('$baseUrl$pathPrefix/search-confirm').replace(
        queryParameters: {'q': q, 'page': '$page', 'pageSize': '$pageSize'},
      );
      final res = await _dio.getUri(uri);
      if (res.statusCode != 200) throw 'HTTP ${res.statusCode}';
  ...
  getHistory({
    ...
    try {
      ...
      final items = (m['items'] as List? ?? const [])
          .map(
            (e) =>
                ProductHistoryItem.fromJson((e as Map).cast<String, dynamic>()),
          )
          .toList();
  ...
  // ======== FAVORITES (idempotentes) ========

  // GET /products/:barcode/favorite  -> { favorited: boolean }
  Future<bool> getFavoriteStatus(String barcode) async {
    try {
      final uri = Uri.parse('$baseUrl$pathPrefix/$barcode/favorite');
      final res = await _getAuthed(uri);
      if (res.statusCode != 200) throw 'HTTP ${res.statusCode}';
      ...
      final msg = e.response?.data is Map
          ? ((e.response!.data['message'] ??
                ...
                .toString()
          : (e.message ?? 'Erro de rede');
      throw 'Falha em GET /products/$barcode/favorite: $msg';
    }
  }

  // PUT /products/:barcode/favorite   -> { favorited: true }
  Future<bool> addFavorite(String barcode) async {
    try {
      final headers = await _authHeaders(requireJwt: true);
      final uri = Uri.parse('$baseUrl$pathPrefix/$barcode/favorite');
      final res = await _dio.putUri(uri, options: Options(headers: headers));
      if (res.statusCode != 200) throw 'HTTP ${res.statusCode}';
      final m = (res.data as Map).cast<String, dynamic>();
      return (m['favorited'] as bool?) ?? true;
    } on DioException catch (e) {
      final msg = e.response?.data is Map
          ? ((e.response!.data['message'] ??
                        e.response!.data['error'] ??
                ...
                .toString()
          : (e.message ?? 'Erro de rede');
      throw 'Falha em PUT /products/$barcode/favorite: $msg';
    }
  }

  // DELETE /products/:barcode/favorite -> { favorited: false }
  Future<bool> removeFavorite(String barcode) async {
    try {
      final headers = await _authHeaders(requireJwt: true);
      final uri = Uri.parse('$baseUrl$pathPrefix/$barcode/favorite');
      final res = await _dio.deleteUri(uri, options: Options(headers: headers));
      if (res.statusCode != 200) throw 'HTTP ${res.statusCode}';
      final m = (res.data as Map).cast<String, dynamic>();
      return (m['favorited'] as bool?) ?? false;
    } on DioException catch (e) {
      final msg = e.response?.data is Map
          ? ((e.response!.data['message'] ??
                ...
                .toString()
          : (e.message ?? 'Erro de rede');
      throw 'Falha em DELETE /products/$barcode/favorite: $msg';
    }
  }

  // GET /products/favorites?q=&page=&pageSize=
  Future<({List<ProductSummary> items, int total, int page, int pageSize})>
  getFavorites({String? q, int page = 1, int pageSize = 20}) async {
    try {
      final qp = <String, String>{
        'page': '$page',
        'pageSize': '$pageSize',
      ...
      final msg = e.response?.data is Map
          ? ((e.response!.data['message'] ??
                ...
                .toString()
          : (e.message ?? 'Erro de rede');
      throw 'Falha em GET /products/favorites: $msg';
    }
  }
}
